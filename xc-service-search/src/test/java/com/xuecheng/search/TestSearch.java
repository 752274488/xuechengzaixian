...
public class TestSearch {
    ...
    //TermQuery  这个是精确查找，不会对查询条件进行分词，而是作为一个整体来匹配索引里面的单词
    @Test
    ...
    //MatchQuery 查询时会把查询条件进行分词 来匹配索引中的分词
    @Test
    public void testMatchQuery() throws IOException, ParseException {
        ...
        /*searchSourceBuilder.query(QueryBuilders.matchQuery("description","spring开发框架")
                        .operator(Operator.OR)); 这个是分的词满足其一即可*/
        searchSourceBuilder.query(QueryBuilders.matchQuery("description","spring开发框架")
                .minimumShouldMatch("80%"));//只搜索满足分词占比80以上的
        //设置源字段过虑,第一个参数结果集包括哪些字段，第二个参数表示结果集不包括哪些字段
        searchSourceBuilder.fetchSource(new String[]{"name","studymodel","price","timestamp"},new String[]{});
        //向搜索请求对象中设置搜索源
        searchRequest.source(searchSourceBuilder);
    ...
    //MultiMatchQuery  进行分词匹配时 会先分词，可以匹配多个域，并且还可以针对某个域进行权重提升
    @Test
    ...
    //BoolQuery 里面有关键词 must should must_not, 它可以组合多个查询条件进行查询
    @Test
    ...
    //filter  过虑是针对搜索的结果进行过虑，过虑器主要判断的是文档是否匹配，不去计算和判断文档的匹配度得分，
