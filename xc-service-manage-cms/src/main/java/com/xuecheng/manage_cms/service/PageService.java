...
public class PageService {
    ...
    
    {
        ...
        //把静态化页面写入Grides
        CmsPage savehtml = savehtml(pageId, pageHtml);
        //发送消息
        sendMessage(pageId);
    ...
    //把静态化页面写入Grides
    public CmsPage savehtml(String pageId,String connect)
    {
        ...
        try {
            //将pagehtml转化成输入流
            InputStream inputStream=IOUtils.toInputStream(connect,"utf-8");
             objectId= gridFsTemplate.store(inputStream, cmsPage.getPageName());
        } catch (IOException e) {
            e.printStackTrace();
        }
        //将objiectId 更新到cmspage
    ...
    {
        CmsPage cmsPage = this.getById(pageId);//这步是因为要获取 站点id所以要判空
        if(cmsPage==null)
        {
            ExceptionCast.cast(CommonCode.INVALID_PARAM);
        }
